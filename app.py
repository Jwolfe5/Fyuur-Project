...
def delete_venue(venue_id):
  ...
  if not error:
    flash(f'Venue {venue_id} is successfully deleted.')
...
def search_artists():
  ...
  for result in search_result:
    data.append({
      "id" : result.id,
      "name" : result.name,
      "num_upcoming_shows" : len(db.session.query(Show).filter(Show.artist.id==result.id).filter(Show.start_time > datetime.now()).all()),
    })
  response={
    "count": len(search_result),
    "data": data
...
def edit_artist_submission(artist_id):
  ...
  if not error:
    flash('Arist was successfully updated!')
...
def edit_venue_submission(venue_id):
  ...
  if not error:
    flash(f'Venue was succesfully updated!')
  # TODO: take values from the form submitted, and update existing
